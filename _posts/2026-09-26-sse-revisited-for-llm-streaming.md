...
> **쉽게 말하면** WebSocket은 **전화**입니다. 양쪽이 아무 때나 말할 수 있지만 전화를 거는 절차가 따로 필요합니다. SSE는 **라디오**입니다. 한 번 주파수를 맞추면 방송국이 계속 보내 주고, 듣는 쪽은 듣기만 합니다. LLM 답변은 "질문 한 번, 답은 길게"라서 라디오면 충분합니다.
{: .prompt-tip }
...
> **쉽게 말하면** SSE 이벤트 하나는 **편지 한 통**입니다. `data:`가 본문, `event:`가 제목, `id:`가 편지 번호이고, **빈 줄이 봉투를 닫는 동작**입니다. 브라우저는 빈 줄을 만나야 "편지 한 통 도착"이라고 앱에 알려 줍니다.
{: .prompt-tip }
...
> **쉽게 말하면** stdio는 **옆자리 동료에게 쪽지를 직접 건네는 것**입니다. 내가 그 사람을 불러 앉혔고(프로세스 실행), 쪽지는 손에서 손으로 갑니다. SSE는 **우편**입니다. 봉투에 주소와 신분증(인증)을 붙여야 하고, 중간에 우체국(프록시)이 끼어 있어서 거기서 지연되거나 모아서 배달될 수 있습니다.
{: .prompt-tip }
...
> **쉽게 말하면** `reasoning_content`는 모델이 답하기 전에 하는 **혼잣말**입니다. "한국어로 답해야겠군, SSE는..." 같은 메모가 먼저 오고, 그다음에 진짜 답(`content`)이 옵니다. 혼잣말은 요금은 나가지만 사용자에게 보여 줄 내용은 아닙니다.
{: .prompt-tip }
...
> **쉽게 말하면** TTFT는 **식당에서 주문하고 첫 접시가 나오기까지의 시간**입니다. 코스 전체가 끝나는 시간은 같아도, 첫 접시가 빨리 나오면 덜 지루합니다. 스트리밍은 요리를 빨리 하는 게 아니라 **나오는 대로 바로 내오는 것**입니다.
{: .prompt-tip }
...
> **쉽게 말하면** SSE는 **접시에 담긴 음식의 모양**이고, MVC와 WebFlux는 **주방이 돌아가는 방식**입니다. 손님은 접시만 봅니다. 주방이 요리사 한 명당 주문 하나를 끝까지 붙잡는 방식(블로킹)인지, 요리사 몇 명이 여러 주문을 오가며 처리하는 방식(논블로킹)인지는 손님 눈에 안 보이지만, 손님이 많아지면 차이가 납니다.
{: .prompt-tip }
...
> **쉽게 말하면** 블로킹 읽기는 **웨이터가 주방 창구 앞에 서서 음식이 나올 때까지 기다리는 것**입니다. 손님 100명이면 웨이터 100명이 창구 앞에 줄을 섭니다. 논블로킹은 **웨이터가 주문만 넣어 두고 다른 테이블을 돌다가, 벨이 울리면 가져오는 것**입니다. 웨이터 몇 명이면 됩니다.
{: .prompt-tip }
...
> **쉽게 말하면** 우체국 규칙에 "본문 첫 글자 앞의 공백 한 칸은 무시한다"가 있다고 생각해 보세요. 보통은 문제가 없습니다. 그런데 LLM은 단어를 보낼 때 **공백을 단어 앞에 붙여서** 보냅니다(`" 서버가"`). 그 공백이 우체국에서 전부 지워지니 단어들이 다 붙어 버립니다. JSON으로 감싸면 본문 첫 글자가 `{`가 되어 규칙에 안 걸립니다.
{: .prompt-tip }
...
> **쉽게 말하면** 버스가 정류장마다 승객을 한 명씩 태우는 게 아니라 **몇 명씩 묶어서** 태우는 것과 같습니다. 이건 정상입니다. 문제는 **종점까지 아무도 안 태우다가 한꺼번에 태우는 경우**이고, 그걸 구분하려면 "몇 번 정차했나"를 세야 합니다.
{: .prompt-tip }
...
> **쉽게 말하면** 버퍼는 **택배 집하장**입니다. 집하장이 있다고 택배가 무조건 늦는 게 아닙니다. 배송 트럭이 비어 있으면 들어온 물건을 바로 실어 보냅니다. 트럭이 꽉 찼을 때(클라이언트가 느릴 때)만 집하장에 물건이 쌓입니다.
{: .prompt-tip }
...
> **쉽게 말하면** gzip은 **압축 포장 담당자**입니다. 물건을 하나씩 포장하면 비효율적이니 상자가 찰 때까지 기다렸다가 한 번에 포장합니다. "이건 급하니 지금 포장해서 보내"라는 지시(flush 표시)가 없으면 계속 기다립니다. 버퍼링이 켜져 있으면 그 지시가 안 붙어서, 마지막 물건이 들어올 때까지 상자를 안 닫습니다.
{: .prompt-tip }
...
> **쉽게 말하면** `X-Accel-Buffering: no`는 택배 상자에 붙이는 **"모아두지 말고 바로 보내 주세요" 스티커**입니다. 집하장 규칙을 바꾸는 게 아니라 이 상자 하나에만 적용됩니다. 그래서 인프라 담당이 아니어도 애플리케이션에서 붙일 수 있습니다.
{: .prompt-tip }
...
> **쉽게 말하면** `proxy_read_timeout`은 **통화 중 침묵이 60초 이어지면 전화를 끊는 교환원**입니다. 통화가 길어서 끊는 게 아니라 **말이 없어서** 끊습니다. 모델이 오래 생각하는 동안이 바로 그 침묵입니다.
{: .prompt-tip }
...
- **SSE는 HTTP 그대로인 단방향 스트림**이라 LLM 토큰 전송의 표준이 됐습니다. 이벤트 하나는 `data:` 줄과 빈 줄로 끝납니다.
- **토큰이 한꺼번에 도착한 원인은 nginx의 `gzip on`**이었습니다. `proxy_buffering on`만으로는 로컬에서 재현되지 않았습니다.
- **해결은 셋 중 하나**입니다. `proxy_buffering off`, `gzip_types`에서 `text/event-stream` 제외, 또는 앱이 `X-Accel-Buffering: no` 헤더를 붙이기.
- **띄어쓰기가 사라진 원인은 SSE 명세의 공백 규칙**이고, 토큰을 JSON으로 감싸면 해결됩니다.
- 스트리밍 장애는 코드보다 **앱 앞뒤에 무엇이 서 있는지**부터 보면 빨리 찾습니다.

...
- **앱은 base URL과 가상 키만 바꿨고 코드는 그대로**입니다. 업스트림 키, 비용, 예산, 폴백은 게이트웨이가 가져갔습니다.
- **무료 모델도 가상 단가를 붙이면 비용이 집계**되고, 그 계산은 토큰 수로 검산이 됩니다.
- **예산 검사는 요청 전에 하므로 한 번은 넘깁니다.** 상태 코드보다 `type: budget_exceeded`로 분기하세요.
- **`/v1` 누락은 404이고, 그다음부터는 쿨다운 때문에 429로 보입니다.** 첫 에러를 찾아야 합니다.
- **무료 → 무료 폴백은 소용이 없습니다.** Orca Router 무료 한도는 계정 단위입니다. 폴백 대상은 한도를 따로 세는 곳이어야 합니다.
- 게이트웨이는 문제를 없애는 게 아니라 **문제가 생기는 자리를 한 곳으로 모아 주는 것**입니다.
